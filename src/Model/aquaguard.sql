...
--
-- Stand-in structure for view `view_fish_kill_predictions`
-- (See below for the actual view)
--
CREATE TABLE `view_fish_kill_predictions` (
`cage_id` int(11)
,`prediction_date` timestamp
,`results` text
,`risk_level` varchar(26)
);

-- --------------------------------------------------------

--
-- Structure for view `view_fish_kill_predictions`
--
DROP TABLE IF EXISTS `view_fish_kill_predictions`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_fish_kill_predictions`  AS SELECT `tbl_processed_records`.`cage_id` AS `cage_id`, `tbl_processed_records`.`date_Inserted` AS `prediction_date`, `tbl_processed_records`.`results` AS `results`, CASE WHEN `tbl_processed_records`.`contamination_level` >= 4 THEN 'High Risk of Fish Kill' WHEN `tbl_processed_records`.`contamination_level` >= 3 THEN 'Moderate Risk of Fish Kill' ELSE 'Low Risk of Fish Kill' END AS `risk_level` FROM `tbl_processed_records` ORDER BY rand() ASC LIMIT 0, 10 ;
